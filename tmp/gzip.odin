...
OS_Name :: #sparse [OS]string{._Unknown = "", .FAT = "FAT", .Amiga = "Amiga", .VMS = "VMS/OpenVMS", .Unix = "Unix", .VM_CMS = "VM/CMS", .Atari_TOS = "Atari TOS", .HPFS = "HPFS", .Macintosh = "Macintosh", .Z_System = "Z-System", .CP_M = "CP/M", .TOPS_20 = "TOPS-20", .NTFS = "NTFS", .QDOS = "QDOS", .Acorn_RISCOS = "Acorn RISCOS", .Unknown = "Unknown"}
...
load_from_file :: proc(
	...
	return file_err == nil ? load_from_bytes(file_data, buf, len(file_data), expected_output_size) : E_General.File_Not_Found
}
...
load_from_bytes :: proc(
	...
	z := &compress.Context_Memory_Input{input_data = data, output = buf}
	return load_from_context(z, buf, known_gzip_size, expected_output_size, allocator)
}

load_from_context :: proc(
	z:   ^$C,
	buf: ^bytes.Buffer,
	known_gzip_size := -1,
	expected_output_size := -1,
