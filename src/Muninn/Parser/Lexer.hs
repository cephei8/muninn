...
import Text.Megaparsec hiding (getSourcePos, State)
...
data PState = PState
  { psAutoSemi      :: !Bool    -- last token triggers auto-semicolons
  , psPendingSemi   :: !Bool    -- virtual semicolon waiting to be consumed
  , psExprLevel     :: !Int     -- nesting depth of ( and [, suppress auto-semi when > 0
  , psEndPos        :: !SrcPos  -- end of last consumed token (before trailing whitespace)
  , psSemiEndPos    :: !SrcPos  -- position to use as end when consuming pending semi
  , psByteOffsets   :: !(UArray Int Int) -- char offset → byte offset mapping
  , psLineStarts    :: !(UArray Int Int) -- line number (1-indexed) → byte offset of line start
  , psPrevTokLine     :: !Int     -- line of previous token (for comment grouping), init 0
  , psCommentGroups   :: [CommentGroup] -- all groups accumulated (reversed)
...
buildLineStarts txt = listArray (1, numLines) starts
  where
    ...
    starts = 0 : [nextOff | (c, nextOff) <- zip chars (tail byteOffs), c == '\n']
...
-- | Raw comment data collected during scLoop, before grouping.
data RawComment = RawComment
  { rcStartPos  :: !SrcPos
  , rcText      :: !Text
  , rcStartLine :: !Int
  , rcEndLine   :: !Int   -- base end line; adjusted post-scLoop
  }

-- | Compute the end_pos of a comment token (matching Odin's end_pos).
commentEndPos :: SrcPos -> Text -> SrcPos
commentEndPos start txt
  | T.length txt >= 2 && T.index txt 1 == '*' =
    -- Block comment: track newlines and columns
    T.foldl' step start txt
  | otherwise =
    -- Line comment: stays on same line
...
sc = do
  ...
  st <- lift get
  remaining <- getInput
  -- Only use the file tag line for nextTokenLine if the tag appears AFTER
  -- all comments (i.e., between comments and the next real token). When
  -- #+build appears before comments, it shouldn't affect nextTokenLine.
  let realNextLine = unPos ln1
  let lastCommentLine = case rawComments of
        (rc:_) -> rcStartLine rc  -- rawComments are in reverse order, head is last
        []     -> 0
  ...
  -- Adjust end lines and group comments, filtering duplicates from backtracking
  unless (null rawComments) $ do
    ...
    let revsRaw = reverse rawComments  -- scLoop collects in reverse
    ...
    scLoop cont ln0 lastOff rcs tagLine tagBefore = do
      ...
      case r of
        ...
        Just (SCTag tl) -> do
          ...
          off <- getOffset
          let newLastOff = if ln == ln0 then Just off else lastOff
          let newTagLine = Just tl  -- always track the last tag line seen
          -- If no comments yet, this tag precedes all comments
...
-- | Adjust end lines: for each raw comment, if the next thing (next comment
-- or next token) starts on a different line than this comment, add 1 to endLine.
...
-- | Two-phase grouping algorithm matching Odin's consume_comment_groups.
groupComments :: Int -> Int -> Bool -> Bool -> Bool -> [RawComment]
              -> ([CommentGroup], Maybe CommentGroup, Maybe CommentGroup)
              -- ^ (all new groups, lineComment, leadComment)
groupComments _ _ _ _ _ [] = ([], Nothing, Nothing)
groupComments prevTokLine nextTokenLine atEof autoSemi tagBefore rcs@(first:_) =
  let -- Phase 1: line comment (same line as previous token)
      -- When autoSemi is active, the auto-semicolon acts as a barrier
      -- after the last same-line comment (matching Odin's behavior where
      -- the inserted ';' token breaks the comment sequence).
      (lineResult, rest1)
        | rcStartLine first == prevTokLine && prevTokLine /= 0 =
            if autoSemi
              then let (grp, endLn, remaining) = consumeSameLine prevTokLine rcs
                   in (Just (grp, endLn), remaining)
              else let (grp, endLn, remaining) = consumeGroup 0 rcs
                   in (Just (grp, endLn), remaining)
        | otherwise = (Nothing, rcs)

      -- Determine if phase-1 group is a line comment
      isLineComment endLn = case rest1 of
        (next:_) -> rcStartLine next /= endLn
      ...
      -- Phase 2: lead comments (remaining comments, group with n=1)
      phase2Results = consumeAllGroups 1 rest1

      -- Last phase-2 group becomes lead comment if endLine+1 >= nextTokenLine.
      -- When a file tag (#+) appears before all comments, don't set lead comment
      -- (matching Odin where docs is captured before the file tag loop).
      leadC
        | tagBefore = Nothing
        | otherwise = case phase2Results of
            [] -> Nothing
            gs -> let (lastG, lastEndLn) = last gs
                  in if lastEndLn + 1 >= nextTokenLine
                       then Just lastG
                       else Nothing

      allGroups = maybe id ((:) . fst) lineResult $ map fst phase2Results
  in (allGroups, lineC, leadC)
  where
...
-- | Consume a group of consecutive comments. n=0 means strict consecutive
-- (no blank lines), n=1 allows 1 blank line gap.
-- Returns (group, endLine for grouping decisions, remaining comments).
consumeGroup :: Int -> [RawComment] -> (CommentGroup, Int, [RawComment])
consumeGroup _ [] = error "consumeGroup: empty list"
consumeGroup n (rc:rcs) = go [rc] (rcEndLine rc) rcs
  where
    go acc endLn (next:rest)
      | rcStartLine next <= endLn + n =
          go (acc ++ [next]) (rcEndLine next) rest
    go acc endLn remaining =
      let comments = map (\r -> Comment (rcStartPos r) (rcText r)) acc
          startP = rcStartPos (head acc)
          lastRc = last acc
          endP = commentEndPos (rcStartPos lastRc) (rcText lastRc)
      in (CommentGroup (SrcSpan startP endP) comments, endLn, remaining)

-- | Like consumeGroup but only includes comments starting on the given line.
-- Used for Phase 1 when auto-semi is active, since the auto-semicolon
-- acts as a barrier after the last same-line comment.
consumeSameLine :: Int -> [RawComment] -> (CommentGroup, Int, [RawComment])
consumeSameLine _ [] = error "consumeSameLine: empty list"
consumeSameLine line (rc:rcs) = go [rc] (rcEndLine rc) rcs
  where
    go acc endLn (next:rest)
...
semi = do
  ...
  if psPendingSemi st
    ...
    else if psAutoSemi st
      then do
        -- Check if next char is '}' or EOF — these trigger auto-semicolons
        -- even without a newline (like Go's semicolon rules)
        atEnd <- MP.atEnd
        if atEnd
...
reservedWords = Set.fromList
  [ "import", "foreign", "package", "typeid", "when", "where"
  , "if", "else", "for", "switch", "in", "not_in", "do"
  , "case", "break", "continue", "fallthrough", "defer", "return"
  , "proc", "struct", "union", "enum", "bit_set", "bit_field"
  , "map", "dynamic", "auto_cast", "cast", "transmute", "distinct"
  , "using", "context", "or_else", "or_return", "or_break", "or_continue"
  , "asm", "inline", "no_inline", "matrix"
  ]

autoSemiKeywords :: Set.Set Text
autoSemiKeywords = Set.fromList
...
pInteger = tok True $ try $ do
  ...
  -- Reject if followed by float indicators, but allow '..' (range operator)
...
numericLit = choice
  [ try hexInt
  , try hexFloatInt  -- 0h prefix: hex-encoded float bits
  , try octInt
  , try binInt
  , decInt
  ]

-- | Parse 0h prefix hex float literal (Odin-specific: hex bits reinterpreted as float)
...
decFloatLit = do
  -- Capture the raw text and parse it.
  -- Note: the Double value is never used (callers only use the raw Text),
  -- so we use fromMaybe 0 for Odin-valid literals that Haskell can't parse (e.g. "1.").
  raw <- try withDot <|> withoutDot
  pure $ fromMaybe 0 (readMaybe (filter (/= '_') (T.unpack raw)))
  where
    withDot = do
      intPart <- takeWhile1P Nothing (\c -> isDigit c || c == '_')
      void $ char '.'
      -- Don't consume the dot if followed by another dot (range operators ..<, ..=, ..>)
      MP.notFollowedBy (char '.')
      -- fracPart can be empty: "1." is a valid float in Odin
      fracPart <- takeWhileP Nothing (\c -> isDigit c || c == '_')
...
hexFloatLit = do
  ...
  -- Note: the Double value is never used (callers only use the raw Text),
  -- so we use fromMaybe 0 for Odin-valid literals that Haskell can't parse (e.g. "0x1p0").
  pure $ fromMaybe 0 (readMaybe (filter (/= '_') (T.unpack raw)))
  where
    withHexDot = do
      void $ char '.'
...
pEscapeChar = choice
  ...
  , pHexEscape 'u' 4
  , pHexEscape 'U' 8
  , anySingle  -- fallback: unrecognized escape, keep char as-is
  ]
