...
instance ToJSON (File SrcSpan) where
    toJSON (File _sp pkg decls comments docs) =
        object
...
instance ToJSON (Stmt SrcSpan) where
    toJSON = \case
        ValueDecl sp attrs names ty vals mut ->
            object
                [ "node" .= ("ValueDecl" :: Text)
                , "pos" .= spanStart sp
                , "end" .= spanEnd sp
...
instance ToJSON (Expr SrcSpan) where
    toJSON = \case
        ...
        TernaryWhenExpr sp cond then_ else_ ->
            object
        ...
        UnionType sp variants params flags ->
            object
                [ "node" .= ("UnionType" :: Text)
                , "pos" .= spanStart sp
                , "end" .= spanEnd sp
        ...
        BitFieldType sp backing fields ->
            object
...
instance ToJSON (Field SrcSpan) where
    toJSON (Field sp names ty def tag flags) =
        object
            [ "node" .= ("Field" :: Text)
            , "pos" .= spanStart sp
            , "end" .= spanEnd sp
...
instance ToJSON (BitFieldField SrcSpan) where
    toJSON (BitFieldField sp name ty size) =
        object
