...
import Text.Megaparsec (try, optional, option, choice, many, (<|>), (<?>))
...
pForeignBlock = do
  ...
  -- Anonymous `foreign { }` gets synthetic Ident("_") with foreign keyword span (Odin reference)
  let lib' = case lib of
        Just e  -> Just e
  ...
  where
    ...
    pathItem = do
      ...
      hasWhen <- optional $ try $ do
        keyword "when"
        _ <- pExpr  -- condition expression
        keyword "else"
        _ <- pString <|> ident
        pure ()
      case hasWhen of
        Just _  -> pure Nothing  -- conditional paths not extractable
        Nothing -> pure base
...
-- Does NOT match expression-level directives (#assert, #panic, #config etc.) —
-- those are handled by pBasicDirective inside pExprOrDeclOrAssign.
...
pDirectiveBlock = do
  ...
  where
    -- Directives set state flags in Odin's AST — inner statement is returned as-is
...
pSwitchStmt = do
  ...
  choice
    ...
    , try $ do
        ...
        body <- pSwitchBody
        sp <- spanFrom start
        -- Odin reference: synthetic `_` at the `switch` keyword position
    ...
    , do exprs <- pExprNoCompLit `sepBy1` comma
         choice
           [ try $ do
               initStmt <- pDeclRest exprs
               semi
...
pHashIdent expected = try $ do
  op "#"
  name <- ident
  if name == expected then pure name
...
pUsingStmt = do
  ...
  choice
    [ try $ do
        exprs <- pExprList
        choice
          [ pDeclRest exprs  -- using name :: ...
          , pAssignRest exprs  -- using name = ...
          , case exprs of
      ...
      if psPendingSemi st
        then choice
          [ try $ do
              ...
              colon
              els <- pExpr' acl  -- allow chained: a ? b : c ? d : e
          ...
          [ try $ do
              ...
              keyword "else"
              els <- pExpr' acl  -- allow chained: A if C1 else B if C2 else D
              pure $ TernaryIfExpr (exprSpan e <-> exprSpan els) cond e els
          , try $ do
              keyword "when"
              cond <- pOrElseExpr' False
              keyword "else"
              els <- pExpr' acl  -- allow chained: A when C1 else B when C2 else D
...
pLogOrExpr' acl = pLeftAssoc (pLogAndExpr' acl) (pLogAndExprRhs acl)
  [ (OpLogOr, op "||") ]

pLogAndExpr' :: Bool -> Parser (Expr SrcSpan)
pLogAndExpr' acl = pLeftAssoc (pCompareExpr' acl) (pCompareExprRhs acl)
  [ (OpLogAnd, op "&&") ]

-- RHS: allow untyped comp lits (e.g. {.Read}) but keep original acl for typed
...
pCompareExprRhs acl = pLeftAssoc (pInExprRhs acl) (pInExprRhs acl)
  [ (OpEq, try $ op "=="), (OpNotEq, try $ op "!="), (OpLtEq, try $ op "<="), (OpGtEq, try $ op ">=")
  , (OpLt, try $ op "<"), (OpGt, try $ op ">")
  , (OpRangeFull, try $ op "..="), (OpRangeHalf, try $ op "..<") ]
pInExprRhs acl = pLeftAssoc (pAddExprRhs acl) (pAddExprRhs acl)
  [ (OpIn, try $ keyword "in"), (OpNotIn, try $ keyword "not_in") ]
pAddExprRhs acl = pLeftAssoc (pMulExprRhs acl) (pMulExprRhs acl)
  [ (OpAdd, try $ op "+"), (OpSub, try $ op "-"), (OpBitOr, try $ op "|"), (OpBitXor, try $ op "~") ]
pMulExprRhs acl = pLeftAssoc (pUnaryExprImpl True acl) (pUnaryExprImpl True acl)
  [ (OpMul, try $ op "*"), (OpQuo, try $ op "/"), (OpModMod, try $ op "%%"), (OpMod, try $ op "%")
  , (OpBitAnd, try $ op "&"), (OpShl, try $ op "<<"), (OpShr, try $ op ">>"), (OpBitAndNot, try $ op "&~") ]

pCompareExpr' :: Bool -> Parser (Expr SrcSpan)
pCompareExpr' acl = pLeftAssoc (pInExpr' acl) (pInExprRhs acl)
  [ (OpEq, try $ op "==")
  , (OpNotEq, try $ op "!=")
  , (OpLtEq, try $ op "<=")
  , (OpGtEq, try $ op ">=")
...
pInExpr' acl = pLeftAssoc (pAddExpr' acl) (pAddExprRhs acl)
  [ (OpIn, try $ keyword "in")
  , (OpNotIn, try $ keyword "not_in")
  ]

pAddExpr' :: Bool -> Parser (Expr SrcSpan)
pAddExpr' acl = pLeftAssoc (pMulExpr' acl) (pMulExprRhs acl)
  [ (OpAdd, try $ op "+")
  , (OpSub, try $ op "-")
  , (OpBitOr, try $ op "|")
  , (OpBitXor, try $ op "~")
...
pMulExpr' acl = pLeftAssoc (pUnaryExpr' acl) (pUnaryExprImpl True acl)
  [ (OpMul, try $ op "*")
  , (OpQuo, try $ op "/")
  , (OpModMod, try $ op "%%")
  , (OpMod, try $ op "%")
...
-- untypedCL: allow untyped comp lit {fields} in pPrimaryExpr
-- typedCL:   allow typed comp lit expr{fields} in pPostfixChain
pUnaryExprImpl :: Bool -> Bool -> Parser (Expr SrcSpan)
pUnaryExprImpl untypedCL typedCL = choice
  [ pUnary "+" OpPos
  , pUnary "-" OpNeg
  , pUnary "!" OpNot
  , try (pUnary "~" OpBitNot)  -- ~ can also be binary
  , try (pUnary "&" OpAddr)    -- & can also be binary
  , pAutoCast
  , pCastExpr
  , pTransmuteExpr
...
pPostfixChain allowCompLit e = choice
  [ try $ do
      op "->"
      selStart <- getPos
      name <- ident
  ...
  , -- Do not parse if pending auto-semicolon (i.e. '[' is on next line)
    try $ do
      st <- lift get
  ...
  , -- Save full PState before trying, restore on failure (try doesn't revert State)
    if allowCompLit then do
      savedSt <- lift get
...
pPrimaryExpr untypedCL typedCL = choice
  ([ pIdentExpr
   , pIntLit
   , pFloatLit
   , pImagLit
   ...
   -- When untypedCL=False: still parse empty `{}` (zero-value comp lit) since
   -- `{` immediately followed by `}` cannot be a block (e.g. `if y & z == {} {`)
   (if untypedCL
...
pDirectiveExpr untypedCL typedCL = do
  ...
  case name of
    ...
    _ -> do
      -- BasicDirective expr is null; parenthesized args handled as CallExpr by pPostfixChain
      nameEnd <- endPos
      let sp = SrcSpan start nameEnd
      pure $ BasicDirective sp name Nothing
  where
    -- Odin's AST doesn't preserve the #simd/#soa/#sparse directive
    pTaggedType _dirStart _name = do
      choice
        [ pArrayOrSliceType' Nothing
        , do -- #soa ^T (pointer with tag)
          pStart <- getPos
          op "^"
    ...
    pMatrixTagExpr dirStart tagName = do
      ...
      -- Wrap as TagExpr(CompLit) not CompLit(TagExpr)
    ...
    -- #relative(i16) ^T → RelativeType(tag=CallExpr(BasicDirective("relative"), [i16]), type=^T)
...
pStructType = do
  ...
  clearPendingSemi
  flEnd <- getPos  -- position AT '}', before consuming it
  rbrace
  sp <- spanFrom start
...
pFieldName = do
  start <- getPos
  name <- identOrKeyword  -- keywords are valid field names in Odin
  sp <- spanFrom start
  pure $ Ident sp name
...
pEnumType = do
  start <- getPos
  keyword "enum"
  -- Backing type must not consume {} (that's the enum body)
...
pBitFieldFieldName = pIdentOnly <|> pKeywordAsUnderscore
  where
    ...
    pKeywordAsUnderscore = do
      start <- getPos
      _ <- identOrKeyword  -- consume the keyword token
      sp <- spanFrom start
      pure $ Ident sp "_"
...
pProcType = do
  ...
  lparen
  paramStart <- getPos  -- after '('
  params <- option [] (map fixParamTypeid <$> (pParam `sepBy1` comma))
  _ <- optional comma
  paramEnd <- getPos  -- at ')'
  rparen
  rparenEnd <- endPos  -- after ')'
  (results, resultsEnd) <- do
    mr <- optional $ try $ do
      op "->"
      choice
        [ try $ do  -- -> ! (diverging/never return type) — reference produces results: null
...
-- Try pProcType first so `proc "c" (int)` is ProcType, not consumed by pProcLit
...
pProcTypeExpr = tryState $ do
  _ <- optional $ try $ do op "#"; ident
  ty <- pProcType
  -- Not followed by block (that would be a proc lit); use MP.single to avoid state changes
  MP.notFollowedBy (void $ MP.single '{')
...
pParam = choice
  ...
  , try $ do
      ...
      restParts <- many $ try $ do
        comma
        _ <- optional (try pFieldFlags)  -- subsequent names may repeat the flag
        s <- getPos; n <- ident; e <- endPos
        pure (n, s, e)
      colon
  ...
  , try $ do
      ...
      case mty of
        Just ty -> do
          -- Odin reference: Ellipsis/Field span is at the type, not at ..
          let tsp = exprSpan ty
...
listSpan (e:es) = exprSpan e <-> exprSpan (last es)
